                ...
                Box(modifier = Modifier.weight(1f)) {
                    if (aiMessages.isEmpty()) {
                        Column(
                            ...
                            Text(
                                text = "Halo! Saya FinMonochrome AI.\\nKetik atau ucapkan sesuatu untuk mulai.",
                    ...
                    LazyColumn(
                        ...
                        items(aiMessages, key = { it.id }) { msg ->
                            ...
                            val tagGoalRegex = Regex(\"\"\"\\[TARGET_NABUNG(_DONE|_CANCEL)?:\\s*([^|]+)\\|\\s*([^|]+)\\|\\s*([^|]+)\\|\\s*([^\\]]+)\\]\"\"\")
                            val tagDepositRegex = Regex(\"\"\"\\[SETOR_TABUNGAN(_DONE|_CANCEL)?:\\s*([^|]+)\\|\\s*([^|]+)\\|\\s*([^\\]]+)\\]\"\"\")
                            val tagTransactionRegex = Regex(\"\"\"\\[CATAT_TRANSAKSI(_DONE|_CANCEL)?:\\s*([^|]+)\\|\\s*([^|]+)\\|\\s*([^|]+)\\|\\s*([^\\]]+)\\]\"\"\")
                                ...
                                Surface(
                                    ...
                                    Column(modifier = Modifier.padding(horizontal = 14.dp, vertical = 10.dp)) {
                                        if (cleanContent.isNotBlank()) {
                                            com.example.ui.components.TypewriterText(
                                                text = cleanContent,
                                                isAi = !isUser,
                                        ...
                                        if (!isUser && matchDeposit != null && onDepositFromAi != null) {
                                            ...
                                            val depositNote = matchDeposit.groupValues[4].trim()
                                            ...
                                            ActionPlanCard(
                                                ...
                                                onExecute = {
                                                    ...
                                                    onUpdateAiMessageContent?.invoke(msg.id, msg.content.replace("[SETOR_TABUNGAN:", "[SETOR_TABUNGAN_DONE:"))
                                                },
                                                onCancel = {
                                                    cancelledDepositMessageIds[msg.id] = true
                                                    onUpdateAiMessageContent?.invoke(msg.id, msg.content.replace("[SETOR_TABUNGAN:", "[SETOR_TABUNGAN_CANCEL:"))
                                                }
                                            )
                                        }
                                        ...
                                        if (!isUser && matchTrans != null && onAddTransactionFromAi != null) {
                                            ...
                                            val transTypeEnum = when {
                                                transTypeStr.contains("INCOME") || transTypeStr.contains("PEMASUKAN") -> com.example.data.model.TransactionType.INCOME
                                                transTypeStr.contains("SAVINGS") || transTypeStr.contains("TABUNGAN") -> com.example.data.model.TransactionType.SAVINGS
                                            ...
                                            ActionPlanCard(
                                                ...
                                                onExecute = {
                                                    onAddGoalFromAi(
                                                        ...
                                                        setOf(2, 3, 4, 5, 6, 7, 1),
                                                        "20:00",
                                                        "Dibuat otomatis dari saran AI FinMonochrome"
                                                    )
